...
class LayoutGenerator:
    ...
    def _combine_images(self, items, axis=1):
        ...
        # Normalize all images to the same number of channels (RGBA)
        max_c = max(img.shape[2] for img in images if img.ndim == 3)
        normalized = []
        for img in images:
            if img.ndim == 3 and img.shape[2] < max_c:
                alpha = np.full((*img.shape[:2], max_c - img.shape[2]), 255, dtype=np.uint8)
